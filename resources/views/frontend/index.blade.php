...
@section('content')
    ...
    <div class="bg-gray-700 border-b-2 border-red-700">
        <div class="max-w-5xl flex flex-col md:flex-row items-center justify-center mx-auto ">
            <img class="mt-6 md:mt-0 mr-0 md:mr-11 w-[90%] md:w-[20%] h-full " src="{{ asset('images/Hero1.jpeg') }}"
                alt="fig" />

            <p class=" w-[80%] px-4 py-4 md:px-10 md:py-10 text-justify text-white text-base">
                Lorem ipsum dolor sit amet consectetur adipisicing elit. Assumenda
                incidunt mollitia excepturi laboriosam distinctio quam doloribus
                tempora nisi consequatur blanditiis inventore laudantium dolore dolor
                dignissimos ipsam, atque dolores sit velit?Lorem ipsum dolor sit amet
                consectetur adipisicing elit. Iusto tempora praesentium quod optio
                quis enim rem eveniet nostrum fuga! Quod officia vel omnis laudantium
                quae dolor deserunt totam nihil quaerat.
            </p>
        </div>
    </div>
    ...
    <div class="max-w-6xl mx-auto pt-20">
        ...
        <div class="mt-10 pb-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 place-items-center">
            @forelse ($tripdetails as $key=>$tripdetail)
                <a href="{{ route('frontend.trip.show', $tripdetail->trip_slug) }}"
    ...
    <div class="bg-gray-100">
        <div class="max-w-6xl mx-auto pt-20">
            <h2 class="text-2xl md:text-4xl text-center md:text-start font-normal">
                Explore by
                <span class="text-black text-2xl md:text-5xl  font-bold">Recommendation</span>
            </h2>

            <div class="mt-10 pb-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 place-items-center">

                @forelse ($recommandations as $key=>$recommandation)
                    <a href="{{ route('frontend.trip.show', $recommandation->trip_slug) }}">
                        <div
                            class="group flex flex-col w-[320px] h-[500px] shadow-2xl cursor-pointer transform transition duration-300 hover:scale-105">
                            <div class="w-full h-[280px] overflow-hidden">
                                <img class="w-full h-full object-cover transition-transform duration-300 group-hover:scale-115"
                                    src="{{ asset('storage/' . $recommandation->sliderimage) }}" alt="" />
                            </div>
                            <h2
                                class="font-bold text-[20px] pt-8 px-6 transition-colors duration-300 group-hover:text-blue-400">
                                {{ $recommandation->trip_name }}
                            </h2>
                            <hr class="text-gray-500 text-2xl mt-6 mb-4 mx-6" />
                            <div class="flex mt-4 space-x-6 px-6 items-center">
                                <img class="rounded-full w-[60px] h-[60px]"
                                    src="{{ asset('storage/' . $recommandation->sliderimage) }}" alt="hgf" />
                                <div class="flex flex-col">
                                    <p class="text-base font-normal text-gray-700">Duration</p>
                                    <p class="text-[22px] text-blue-400 font-bold">{{ $recommandation->trip_duration }}</p>
                                </div>
                            </div>
                        </div>
                    </a>
    ...
    <div class="container  mt-[140px] max-w-6xl mx-auto px-6 mb-10">
        <div class="flex items-center justify-between ">
            ...
            <a href="{{ route('frontend.blogs') }}"
                class=" bg-blue-500 hover:bg-blue-900 px-4 py-4 rounded-lg cursor-pointer hover:text-white text-gray-900 text-base">Explore
                All</a>
        </div>
