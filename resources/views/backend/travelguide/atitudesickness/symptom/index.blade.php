        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-lg overflow-hidden border border-gray-200 dark:border-gray-700">
            ...
            <div class="bg-gradient-to-r from-blue-600 to-indigo-700 px-6 py-8">
                <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 class="text-2xl font-bold text-white">Altitude Sickness Symptoms</h1>
                    ...
                    <a class="bg-white/10 hover:bg-white/20 text-white px-5 py-2.5 rounded-lg transition duration-300 flex items-center gap-2 shadow-md backdrop-blur-sm"
                       href="{{ route('admin.altitudesickness.create') }}">
            ...
            <div class="p-6">
                ...
                @if($symptoms->isEmpty())
                    <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-8 text-center">
                        ...
                        <h3 class="mt-4 text-lg font-medium text-gray-800 dark:text-gray-200">No Symptoms Found</h3>
                        <p class="mt-1 text-gray-600 dark:text-gray-400">Get started by adding your first symptoms set</p>
                    </div>
                @else
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Mild Symptoms</th>
                                    <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Severe Symptoms</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                @foreach ($symptoms as $symptom)
                                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800 dark:text-gray-200">{{ $symptom->title }}</td>
                                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                                            <ul class="space-y-1">
                                                @foreach ($symptom->mild_symptoms as $mild_symptom)
                                                    <li class="flex items-start">
                                                        <svg class="h-4 w-4 text-blue-500 mr-2 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                                                        </svg>
                                                        <span>{{ $mild_symptom }}</span>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </td>
                                        <td class="px-6 py-4 text-sm text-gray-600 dark:text-gray-300">
                                            <ul class="space-y-1">
                                                @foreach ($symptom->severe_symptoms as $severe_symptom)
                                                    <li class="flex items-start">
                                                        <svg class="h-4 w-4 text-red-500 mr-2 mt-0.5 flex-shrink-0" viewBox="0 0 20 20" fill="currentColor">
                                                            <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd" />
                                                        </svg>
                                                        <span>{{ $severe_symptom }}</span>
                                                    </li>
                                                @endforeach
                                            </ul>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm">
                                            <div class="flex space-x-2">
                                                <a href="{{ route('admin.altitudesickness.edit', $symptom) }}" 
                                                ...
                                                <form action="{{ route('admin.altitudesickness.destroy', $symptom) }}" method="POST">
                                                    ...
                                                    <button type="submit" 
                                                            ...
                                                            title="Delete"
                                                            onclick="return confirm('Are you sure you want to delete these symptoms?')">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
