...
@section('content')
    ...
    <div class="bg-gray-800 rounded-lg shadow p-6 mt-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl sm:text-2xl font-bold">Hero Section Details</h2>
            <a class="bg-blue-600 hover:bg-blue-700 text-white px-2 text-[10px] sm:text-base sm:px-4 py-2 rounded"
                href="{{ route('admin.herosections.create') }}">
                Create Hero Section
            </a>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-700">
                    <tr>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Hero
                            Section
                            Title
                        </th>
                ...
                <tbody class="bg-gray-800 divide-y divide-gray-700">
                    @forelse($herosections as $herosection)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $herosection->herotitle }}</td>
                            <td class="px-6 py-4 whitespace-nowrap"><img class="rounded-2xl" width="100px"
                                    src="{{ asset('storage/' . $herosection->heroimage) }}" alt=""></td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a class="text-blue-400 hover:text-blue-300 mr-3"
                                    href="{{ route('admin.herosections.edit', $herosection->id) }}">Edit</a>
                                <form class="inline" action="{{ route('admin.herosections.destroy', $herosection->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a class="text-green-400 hover:text-green-300"
                                    href="{{ route('admin.herosections.show', $herosection->id) }}">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td class="text-center py-4 text-gray-400" colspan="6"> hero Section not available.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            <div class="mt-6 flex justify-center">
                {{ $herosections->links('pagination::tailwind') }}
            </div>
        </div>
    </div>
@endsection
