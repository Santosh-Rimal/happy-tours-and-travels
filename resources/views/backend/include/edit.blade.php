...
@section('title', 'Edit Trip Include')

@section('content')
    <div class="bg-gray-800 rounded-lg shadow p-6 max-w-4xl mx-auto">
        <h2 class="text-2xl font-bold mb-6">Edit Trip Include</h2>
        ...
        <form class="space-y-6" action="{{ route('admin.includes.update', $include->id) }}" method="POST"
                ...
                <select
                    ...
                    @foreach ($trips as $id => $title)
                        <option value="{{ $title->id }}"
                            {{ old('trip_detail_id', $include->trip_detail_id) == $id ? 'selected' : '' }}>
                            {{ $title->trip_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <fieldset>
                ...
                <div class="bg-gray-700 rounded-lg p-6">
                    <div id="includes-container">
                        @foreach (old('includes', $include->includes ?? ['']) as $index => $item)
                            <div class="include-item mb-3 flex items-center">
                                <input
                                    ...
                                    type="text" name="includes[]" value="{{ $item }}" required
