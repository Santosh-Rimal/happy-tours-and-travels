...
@section('content')
    ...
    <div class="bg-gray-800 rounded-lg shadow p-6 max-w-6xl mx-auto">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-2xl font-bold text-white mb-6">Trip includes List</h2>
            <a class="bg-blue-600 hover:bg-blue-700 text-white px-2 text-[10px]  sm:text-base sm:px-4 py-2 rounded"
                href="{{ route('admin.includes.create') }}">
                Create Trip Inclusion
            </a>
        </div>
            ...
            <table class="min-w-full table-auto border border-gray-600 text-white">
                <thead class="bg-gray-700">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left border-b border-gray-600">includes</th>
                        <th class="px-4 py-3 text-left border-b border-gray-600">Actions</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($includes as $index => $include)
                        <tr class="bg-gray-700/30 hover:bg-gray-700/50 transition-colors">
                            <td class="px-4 py-3 border-b border-gray-600">{{ $index + 1 }}</td>
                            <td class="px-4 py-3 border-b border-gray-600">{{ $include->tripDetail->trip_name ?? 'N/A' }}
                            </td>
                            <td class="px-4 py-3 border-b border-gray-600">
                                @if (is_array($include->includes))
                                    <ul class="list-disc pl-5 space-y-1">
                                        @foreach ($include->includes as $item)
                                            <li>{{ $item }}</li>
                                        @endforeach
                                    </ul>
                                @else
                            ...
                            <td class="px-4 py-3 border-b border-gray-600">
                                <div class="flex space-x-2">
                                    <a class="text-blue-400 hover:text-blue-300"
                                        href="{{ route('admin.includes.edit', $include->id) }}">Edit</a>

                                    <a class="text-green-400 hover:text-green-300"
                                        href="{{ route('admin.includes.show', $include->id) }}">Show</a>
                                    <form class="inline" action="{{ route('admin.includes.destroy', $include->id) }}"
                                        method="POST">
                                        @csrf
                                        @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td class="text-center py-4 text-gray-400" colspan="6">Trip Inclusion not available.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
