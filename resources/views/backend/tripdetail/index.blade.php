    ...
    <div class="bg-gray-800 rounded-lg shadow p-6 mt-8">
        <div class="flex justify-between items-center mb-6">
            ...
            <a class="bg-blue-600 hover:bg-blue-700 text-white px-2 text-[10px] sm:text-base sm:px-4 py-2 rounded"
                href="{{ route('admin.tripdetails.create') }}">
                Create New Trip
            </a>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-700">
                <thead class="bg-gray-700">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">Price
                        </th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                            Difficulty</th>
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-300 uppercase tracking-wider">
                ...
                <tbody class="bg-gray-800 divide-y divide-gray-700">
                    @forelse($tripdetails as $trip)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $trip->trip_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $trip->category->name }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                ...
                                <form class="inline" action="{{ route('admin.tripdetails.destroy', $trip->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a class="text-green-400 hover:text-green-300"
                                    href="{{ route('admin.tripdetails.show', $trip->id) }}">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
