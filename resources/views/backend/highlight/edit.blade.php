...
@section('content')
    <div class="bg-gray-800 rounded-lg shadow p-6 max-w-4xl mx-auto">
        <h2 class="text-2xl font-bold mb-6">Edit Trip highlight</h2>
        ...
        <form class="space-y-6" action="{{ route('admin.highlights.update', $highlight->id) }}" method="POST"
                ...
                <select
                    ...
                    @foreach ($trips as $id => $title)
                        <option value="{{ $title->id }}"
                            {{ old('trip_detail_id', $highlight->trip_detail_id) == $title->id ? 'selected' : '' }}>
                            {{ $title->trip_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <fieldset>
                ...
                <div class="bg-gray-700 rounded-lg p-6">
                    <div id="highlights-container">
                        @foreach (old('highlights', $highlight->highlights ?? ['']) as $index => $item)
                            <div class="highlight-item mb-3 flex items-center">
                                <input
                                    class="flex-1 bg-gray-600 border @error('highlights.' . $index) border-red-500 @else border-gray-500 @enderror rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    type="text" name="highlights[]" value="{{ $item }}" required
