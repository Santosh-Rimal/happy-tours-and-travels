            ...
            <table class="min-w-full divide-y divide-gray-700">
                ...
                <tbody class="bg-gray-800 divide-y divide-gray-700">
                    @forelse($itineraries as $itinerary)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $itinerary->tripDetail->trip_name }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $itinerary->title }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a class="text-blue-400 hover:text-blue-300 mr-3"
                                    href="{{ route('admin.itineraries.edit', $itinerary->id) }}">Edit</a>
                                <form class="inline" action="{{ route('admin.itineraries.destroy', $itinerary->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a class="text-green-400 hover:text-green-300"
                                    href="{{ route('admin.itineraries.show', $itinerary->id) }}">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
