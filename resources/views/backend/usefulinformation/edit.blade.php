...
@section('content')
    ...
    <div class="bg-gray-800 rounded-lg shadow p-6 max-w-4xl mx-auto">
        <h2 class="text-2xl font-bold mb-6">Edit Trip information</h2>
        ...
        <form class="space-y-6" action="{{ route('admin.usefulinformations.update', $usefulinformation->id) }}" method="POST"
            ...
            <fieldset>
                ...
                <div class="bg-gray-700 rounded-lg p-6 space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="text" name="title" value="{{ old('title', $usefulinformation->title) }}"
                                required>
                            @error('title')
                            ...
                            <select
                                ...
                                @foreach ($trips as $trip)
                                    <option value="{{ $trip->id }}"
                                        {{ old('trip_detail_id', $usefulinformation->trip_detail_id) == $trip->id ? 'selected' : '' }}>
                                        {{ $trip->trip_name }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div>
                        ...
                        <textarea
                            class="ckeditor border @error('description') border-red-500 @else border-gray-500 @enderror rounded-md px-3 py-2 text-white w-full"
                            name="description">{{ old('description', $usefulinformation->description) }}</textarea>
                        @error('description')
                            <span class="text-red-400 text-sm mt-1 block">{{ $message }}</span>
                        @enderror
                    </div>
