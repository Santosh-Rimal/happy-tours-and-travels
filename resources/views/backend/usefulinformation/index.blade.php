...
@section('content')
    ...
    <div class="bg-gray-800 rounded-lg shadow p-6 mt-8">
        <div class="flex justify-between items-center mb-6">
            <h2 class="text-xl sm:text-2xl font-bold">Trip Details</h2>
            <a class="bg-blue-600 hover:bg-blue-700 text-white px-2 text-[10px] sm:text-base sm:px-4 py-2 rounded"
                href="{{ route('admin.usefulinformations.create') }}">
                Create New Trip Useful informations
            </a>
        </div>
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-700">
                ...
                <tbody class="bg-gray-800 divide-y divide-gray-700">
                    @forelse($usefulinformations as $usefulInformation)
                        <tr>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $usefulInformation->title }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">{{ $usefulInformation->description }}</td>
                            <td class="px-6 py-4 whitespace-nowrap">
                                <a class="text-blue-400 hover:text-blue-300 mr-3"
                                    href="{{ route('admin.usefulinformations.edit', $usefulInformation->id) }}">Edit</a>
                                <form class="inline"
                                    action="{{ route('admin.usefulinformations.destroy', $usefulInformation->id) }}"
                                    method="POST">
                                    @csrf
                                    @method('DELETE')
                                ...
                                <a class="text-green-400 hover:text-green-300"
                                    href="{{ route('admin.usefulinformations.show', $usefulInformation->id) }}">View</a>
                            </td>
                        </tr>
                    @empty
                        <tr>
