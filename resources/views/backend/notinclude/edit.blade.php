...
@section('title', 'Edit Trip Not Include')

@section('content')
    <div class="bg-gray-800 rounded-lg shadow p-6 max-w-4xl mx-auto">
        <h2 class="text-2xl font-bold mb-6">Edit Trip Not Include</h2>
        ...
        <form class="space-y-6" action="{{ route('admin.notincludes.update', $notinclude->id) }}" method="POST"
                ...
                <select
                    ...
                    @foreach ($trips as $id => $title)
                        <option value="{{ $title->id }}"
                            {{ old('trip_detail_id', $notinclude->trip_detail_id) == $title->id ? 'selected' : '' }}>
                            {{ $title->trip_name }}
                        </option>
                    @endforeach
                </select>
            ...
            <!-- Trip Not not Section -->
            <fieldset>
                <legend class="text-xl font-semibold flex items-center space-x-3">
                    <svg class="w-6 h-6 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <span>Trip Not not</span>
                </legend>

                <div class="bg-gray-700 rounded-lg p-6">
                    <div id="notincludes-container">
                        @foreach (old('notincludes', $notinclude->notincludes ?? ['']) as $index => $item)
                            <div class="notinclude-item mb-3 flex items-center">
                                <input
                                    class="flex-1 bg-gray-600 border @error('notincludes.' . $index) border-red-500 @else border-gray-500 @enderror rounded-md px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                                    type="text" name="notincludes[]" value="{{ $item }}" required
                                    placeholder="Enter not include feature">
                                @if ($index > 0)
                                    <button class="remove-notinclude ml-2 text-red-400 hover:text-red-300" type="button">
                    ...
                    <button class="mt-2 flex items-center text-blue-400 hover:text-blue-300" id="add-notinclude"
                        type="button">
                        <svg class="w-5 h-5 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                d="M12 6v6m0 0v6m0-6h6m-6 0H6" />
                        </svg>
                        Add Another Not Include
                    </button>
                </div>
            </fieldset>
...
@push('scripts')
    <script>
        document.getElementById('add-notinclude').addEventListener('click', function() {
            const container = document.getElementById('notincludes-container');
            const newInput = document.createElement('div');
            newInput.className = 'notinclude-item mb-3 flex items-center';
            newInput.innerHTML = `
                <input
                    ...
                    type="text" name="notincludes[]" required
                    placeholder="Enter not include feature">
                <button class="remove-notinclude ml-2 text-red-400 hover:text-red-300" type="button">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        document.getElementById('notincludes-container').addEventListener('click', function(e) {
            if (e.target.closest('.remove-notinclude')) {
                e.target.closest('.notinclude-item').remove();
            }
        });
    </script>
@endpush
