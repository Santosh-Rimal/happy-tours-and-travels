...
@section('content')
    <div class="bg-gray-800 rounded-lg shadow p-6 max-w-4xl mx-auto">
        <h2 class="text-2xl font-bold mb-6">Create Trip NotInclusion</h2>
        ...
        <form class="space-y-6" action="{{ route('admin.notincludes.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            ...
            <!-- Trip NotIncludes Section -->
            <fieldset>
                <legend class="text-xl font-semibold flex items-center space-x-3">
                    <svg class="w-6 h-6 text-yellow-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7" />
                    </svg>
                    <span>Trip NotIncludes</span>
                </legend>

                <div class="bg-gray-700 rounded-lg p-6">
                    <div id="not-includes-container">
                        @foreach (old('notincludes', ['']) as $index => $notinclude)
                            <div class="not-include-item mb-3 flex items-center">
                                <input
                                    ...
                                    type="text" name="notincludes[]" value="{{ $notinclude }}" required
...
@push('scripts')
    <script>
        ...
        document.getElementById('add-include').addEventListener('click', function() {
            const container = document.getElementById('not-includes-container');
            const newInput = document.createElement('div');
            newInput.className = 'not-include-item mb-3 flex items-center';
            newInput.innerHTML = `
            <input
        ...
        document.getElementById('not-includes-container').addEventListener('click', function(e) {
            if (e.target.closest('.remove-notinclude')) {
                e.target.closest('.not-include-item').remove();
            }
        });
    </script>
@endpush
